...
async def travel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    taskTypeTravel = Task.TRAVEL.value
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    messageTasks = '\n'.join([f"{task.owner} is {task.type.description} [{task.time_remaind()}]" for task in tasks if not task.type.passive])
    messageTasks += '\n'.join([f"{task.owner} is {task.type.description}..." for task in tasks if task.type.passive])
    if messageTasks:
...
def has_task(owner: str):
    return owner in repo.get_all_progress_task_by_owner(owner)
